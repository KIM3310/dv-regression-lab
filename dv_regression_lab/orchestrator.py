...
def run_suite(suite_path: Path, store: RunStore) -> RegressionRun:
    ...
    requested_at = dt.datetime.now(dt.timezone.utc).replace(microsecond=0).isoformat() + "Z"
    ...
    completed_at = dt.datetime.now(dt.timezone.utc).replace(microsecond=0).isoformat() + "Z"
