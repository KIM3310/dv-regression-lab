# Conversion UX Model - DV Regression Lab
...
| Primary buyer or user | ASIC verification managers, EDA platform teams, CI owners, and silicon program leads. |
| Value wedge | DV teams need a clearer loop from nightly regression evidence to flaky-test triage and operator handoff. |
...
| First action | Open the README archived-status note and enterprise-readiness note, then inspect Regression examples remain synthetic or permission-safe only if this older domain proof is relevant. |
...
- Optional entry: Regression triage assessment ($8k-$20k) when this archived proof exactly matches the buyer problem.
- Revival step: DV control-tower pilot ($40k-$150k) only after the active successor and support boundary are clear.
- Longer-term path: EDA workflow extension ($100k+/year) if the repository is refreshed and assigned an owner.

## Pricing Frame

- Do not price the archive as a maintained product.
- Price the revived diagnostic, workshop, template, or implementation path.
- Use Assessment calls only as a discussion metric until fresh validation exists.
