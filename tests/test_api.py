...
def test_api_runs_suite_and_returns_triage(tmp_path):
    ...
    assert meta_response.json()["links"]["proof_map"] == "/v1/proof-map"
    ...
    proof_map_response = client.get("/v1/proof-map")
    assert proof_map_response.status_code == 200
    proof_map = proof_map_response.json()
    assert proof_map["contract_version"] == "dv-regression-proof-map-v1"
    assert proof_map["reviewer_fast_path"][0] == "/v1/proof-map"
